...
export default function WatchPage() {
    ...
    const { data: details, loading: detailsLoading, error: detailsError} = useFetch(async () => await getMovieDetail(slug!))
            ...
            {detailsLoading ? (
                ...
                <div>
                    <div className="relative w-full" style={{ paddingTop: '56.25%' }}>
                        <iframe
                            src={currentEspisode as string || data?.episodes[0].server_data[0].link_embed as string}
                    ...
                    {data?.episodes.map((item, index) => (
                        <div key={index} className="my-5 bg-[#111828] p-4 rounded-xl">
                            <h2 className="font-semibold text-white font-playwight">{item.server_name}</h2>
                            <div className="mt-2 flex flex-wrap gap-2 max-h-[300px] overflow-y-auto">
