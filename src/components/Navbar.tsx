...
export default function Navbar() {
  ...
  const menuItems = [
    ...
    { label: "Phim Bộ", icon: <Clapperboard size={16} />, path: "/movie/phim-bo" },
    { label: "Phim Lẻ", icon: <Film size={16} />, path: "/movie/phim-le" },
    { label: "TV Shows", icon: <Tv size={16} />, path: "/movie/tv-shows" },
    { label: "Hoạt Hình", icon: <PawPrint size={16} />, path: "/movie/hoat-hinh" },
    {
      label: "Thể Loại",
  ...
  return (
    <div className="w-full h-14 bg-opacity-85 bg-[#24252a] flex items-center px-5 text-white fixed z-50 top-0">
      {/* Desktop Navbar */}
      <div className="max-w-7xl mx-auto w-full hidden md:flex items-center justify-between">
        ...
        >
          ...
          <span className="ml-3 text-white font-medium text-lg">FlickNest</span>
        </div>
      ...
      <div className="w-full flex md:hidden items-center justify-between">
        <div className="flex items-center gap-1" onClick={() => navigate("/")}>
          <img src={logo} alt="logo" />
          <span className="ml-2 font-semibold text-white">FlickNest</span>
        </div>
      ...
      {/* Drawer */}
      <div
        className={`fixed top-0 right-0 h-full w-[80vw] bg-[#1f1f1f] text-white z-50 transform transition-transform duration-300 ease-in-out ${
